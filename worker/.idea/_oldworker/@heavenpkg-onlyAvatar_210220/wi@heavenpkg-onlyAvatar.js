// MONSTER
const TITLE = 'HEAVENPKG-ONLYAVATAR';
const APIKEY = 'uUI8kZX3Nx0TNfL4rnuQYJuuly2E0QpX';
const INTERVAL = 500;
...
ipc.connectTo('listener', () => {
    ipc.of['listener'].on('connect', () => {
        ...
        if(worker._prevHash) ipc.of['listener'].emit(TITLE, worker._snapshot);
    });
});
...
let scheduler_test = (task, param) => setInterval(task, INTERVAL, param, apiInstance);
